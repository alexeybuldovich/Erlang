...
handle(Req, State) ->
    ...
    Res_json = jsx:encode([{<<"result">>,<<"ok">>}, {<<"response">>, Result}]),
...
get_response() ->
	...
	receive
        ...
        %jsx:encode([<<"result">>, Msg]);
			  io:format("~n Recieve1: ~p~n",[Msg]),
        Msg;
    {Msg} ->
        io:format("~n Receive2: ~p~n", [Msg])
	end.
